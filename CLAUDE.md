...
- **All content comes from `src/data/resume.mjs`** — the website components (`Header`, `Summary`, `Skills`, `Experience`, `Education`, `Contact`) import `personal`, `headline`, `highlights`, `skillGroups`, `experiences` and `education` from it, the same data behind the PDF/DOCX. Edit `resume.mjs`, not the components, to change what the site says. The only content still local to a component is the logo grid in `Skills.js` (featured tools with their logo URLs).
